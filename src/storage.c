...
static UINT storage_media_open_internal(void)
{
    ...
    /* =====================================================================
       MÁQUINA DE FORMATAÇÃO: Mude para 1 na próxima compilação para formatar!
       ===================================================================== */
    #define FORMAT_DRIVE_NOW 0

    #if FORMAT_DRIVE_NOW
        /* 1. Apaga fisicamente a QSPI (O LevelX auto-formata-se ao ver 0xFF) */
        ...
        while (in_progress) {
            ...
            tx_thread_sleep(1); /* Mantém o RTOS vivo */
        }
        g_qspi0.p_api->close(g_qspi0.p_ctrl);

        /* 2. Cria a Tabela FAT do FileX para suportar fotos e ficheiros */
...
{
    if (g_storage_mutex_ready) tx_mutex_get(&g_storage_mutex, TX_WAIT_FOREVER);
}

static void storage_unlock(void)
{
    if (g_storage_mutex_ready) tx_mutex_put(&g_storage_mutex);
}

static void storage_copy_text(char *dest, size_t dest_size, const char *src, size_t src_len)
{
    if ((NULL == dest) || (0U == dest_size)) return;
    if (NULL == src) { dest[0] = '\0'; return; }
    if (src_len >= dest_size) src_len = dest_size - 1U;
...
static bool storage_parse_users(const char *json_buffer)
{
    ...
    while ((ptr = strstr(ptr, "\"name\"")) != NULL && g_user_count < MAX_USERS)
    {
        ...
        if (NULL == name_start) break;
        name_start++;

        const char *name_end = strchr(name_start, '"');
        if (NULL == name_end) break;

        const char *uid_key = strstr(name_end, "\"uid\"");
        if (NULL == uid_key) break;

        const char *uid_start = strchr(uid_key + 5, '"');
        if (NULL == uid_start) break;
        uid_start++;

        const char *uid_end = strchr(uid_start, '"');
        if (NULL == uid_end) break;

        storage_copy_text(g_users[g_user_count].name, sizeof(g_users[g_user_count].name), name_start, (size_t) (name_end - name_start));
        storage_copy_text(g_users[g_user_count].uid, sizeof(g_users[g_user_count].uid), uid_start, (size_t) (uid_end - uid_start));
...
static bool storage_prepare_users_json_locked(char *json_buffer, size_t json_buffer_size, size_t *json_size)
{
    ...
    if ((NULL == json_buffer) || (NULL == json_size) || (0U == json_buffer_size)) return false;

    json_buffer[offset++] = '[';

    if ((0 == count_to_write) && g_recent_user_valid)
    {
        int written = snprintf(&json_buffer[offset], json_buffer_size - offset,
                               "{\"name\":\"%s\",\"uid\":\"%s\"}", g_recent_user.name, g_recent_user.uid);
        if ((written < 0) || ((size_t) written >= (json_buffer_size - offset))) return false;
    ...
    for (int i = 0; i < count_to_write; i++)
    {
        int written = snprintf(&json_buffer[offset], json_buffer_size - offset,
                               "%s{\"name\":\"%s\",\"uid\":\"%s\"}",
                               (i == 0) ? "" : ",", g_users[i].name, g_users[i].uid);
        if ((written < 0) || ((size_t) written >= (json_buffer_size - offset))) return false;
        offset += (size_t) written;
    }

    if (offset >= (json_buffer_size - 2U)) return false;
...
/* =========================================================================
   GRAVAÇÃO E LEITURA USANDO O FILEX (Sistema de Ficheiros Real)
   ========================================================================= */
static bool storage_save_users_locked(void)
{
    ...
    if (FX_SUCCESS != status)
    {
        ...
        if ((FX_SUCCESS != status) && (FX_ALREADY_CREATED != status)) return false;

        status = fx_file_open(&g_fx_media0, &g_users_file, "users.json", FX_OPEN_FOR_WRITE);
        if (FX_SUCCESS != status) return false;
    }

    status = fx_file_truncate(&g_users_file, 0U);
    if (FX_SUCCESS == status) status = fx_file_seek(&g_users_file, 0U);
    if (FX_SUCCESS == status) status = fx_file_write(&g_users_file, g_storage_persist_json, g_storage_persist_json_size);
    if (FX_SUCCESS == status) status = fx_file_close(&g_users_file);
    else {
        fx_file_close(&g_users_file);
        return false;
    }

    if (FX_SUCCESS == status) status = fx_media_flush(&g_fx_media0);
...
static void storage_load_users_locked(void)
{
    ...
    if (g_storage_loaded || !storage_media_access_allowed() || !storage_media_open()) return;
    ...
    if (FX_SUCCESS == status)
    {
        ...
        if (FX_SUCCESS == status)
        {
            ...
            storage_parse_users(json_buffer);
        }
        fx_file_close(&g_users_file);
    }
    else
...
void storage_init(void)
{
    ...
    if (!g_storage_mutex_ready) {
        if (TX_SUCCESS == tx_mutex_create(&g_storage_mutex, "storage_mutex", TX_NO_INHERIT)) g_storage_mutex_ready = true;
    }

    if (!g_storage_thread_ready) {
        if (TX_SUCCESS == tx_semaphore_create(&g_storage_persist_semaphore, "storage_persist", 0U)) {
            if (TX_SUCCESS == tx_thread_create(&g_storage_thread, "storage_thread", storage_thread_entry, 0U,
                                               g_storage_thread_stack, sizeof(g_storage_thread_stack),
                                               14U, 14U, TX_NO_TIME_SLICE, TX_AUTO_START)) {
    ...
    if (g_storage_thread_ready) tx_semaphore_put(&g_storage_persist_semaphore);
}
...
static void storage_thread_entry(ULONG initial_input)
{
    ...
    while (1)
    {
        ...
        {
            bool ok = false;
            while (!storage_media_access_allowed()) tx_thread_sleep(10U);

            storage_lock();
            if (g_storage_persist_requested)
            {
                g_storage_persist_requested = false;
                if ((0 == g_user_count) && !g_recent_user_valid) storage_load_users_locked();
                if (storage_media_open()) ok = storage_save_users_locked();
...
{
    bool found = false;
    if ((NULL == uid_str) || (NULL == out_name)) return false;
...
bool storage_user_get(int index, char *out_name, char *out_uid)
{
    ...
    if ((index >= 0) && (index < g_user_count)) {
        if (NULL != out_name) storage_copy_text(out_name, NAME_MAX_LEN, g_users[index].name, strlen(g_users[index].name));
        if (NULL != out_uid) storage_copy_text(out_uid, UID_MAX_LEN, g_users[index].uid, strlen(g_users[index].uid));
        ok = true;
    }
...
{
    bool ok = false;
    if ((NULL == uid_str) || (NULL == name) || ('\0' == uid_str[0]) || ('\0' == name[0])) return false;
...
{
    bool ok = false;
    if ((NULL == uid_str) || ('\0' == uid_str[0])) return false;
    ...
    {
        if (0 == strcmp(g_users[i].uid, uid_str)) {
            for (int j = i; j < (g_user_count - 1); j++) g_users[j] = g_users[j + 1];
...
{
    storage_init();
    if (!g_storage_thread_ready) return false;
